<x-app-layout>
    ...
    <div class="flex bg-gray-100">
        ...
        <div class="text-indigo-800 text-left px-4 py-4 mg-2">
            ...
            <!-- ++++ GROUP_FORM::START -->
            <form action="{{ url('supervisor_group/update') }}" method="POST" class="w-full max-w-lg">
                @csrf
                    ...
                    <div class="w-full md:w-1/1 px-3 py2">
                        ...
                        <div class="form-group">
                            <select name="supervisor_user_id" id="supervisor-user-id" class="form-control">
                                <option value="{{$wip_supervisor_groups->supervisor_user_id}}">{{$wip_supervisor_groups->supervisor_user_id}}</option>
                                @foreach($wip_users as $wip_user)
                                    <option value="{{$wip_user->user_id}}">{{ $wip_user->name . "(" . $wip_user->user_id . ")" }}</option>
                                @endforeach
                            </select>
                        </div>  
                        <!-- <input name="supervisor_user_id" value="{{$wip_supervisor_groups->supervisor_user_id}}" class="appearance-none block w-full text-blue-900 border border-gray-600 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" type="text" placeholder=""> -->
                    </div>
                    <!-- +++ COL::group_id+++ -->
                    <div class="w-full md:w-1/1 px-3 py-2">
                        ...
                        <div class="form-group">
                            <select name="group_id" id="group-id" class="form-control">
                                <option value="{{$wip_supervisor_groups->group_id}}">{{$wip_supervisor_groups->group_id}}</option>
                                @foreach($wip_groups as $wip_group)
                                    <option value="{{$wip_group->group_id}}">{{ $wip_group->group_display_name . "(" . $wip_group->domain_id . ")" }}</option>
                                @endforeach
                            </select>
                        </div>                         
                        <!-- <input name="group_id" value="{{$wip_supervisor_groups->group_id}}" class="appearance-none block w-full text-blue-900 border border-gray-600 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" type="text" placeholder=""> -->
                    </div>
                </div> 
                <!-- +++ COL::BUTTON +++ -->
                ...
                <input type="hidden" name="supervisor_group_id" value="{{$wip_supervisor_groups->supervisor_group_id}}">
            </form>
            <!-- ++++ GROUP_FORM::EMD -->
        </div>
        <!-- ######## LEFT_AREA::END ########## -->
