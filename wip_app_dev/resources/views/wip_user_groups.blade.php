<x-app-layout>
    ...
    <div class="flex bg-gray-100">
        ...
        <div class="text-indigo-800 text-left px-4 py-4 mg-2">
            ...
            <!-- ++++ GROUP_FORM::START -->
            <form action="{{ url('user_group') }}" method="POST" class="w-full max-w-lg">
                @csrf
                <div class="flex flex-col px-2 py-2">
                    ...
                    <div class="w-full md:w-1/1 px-3 py2">
                        ...
                        <!-- <input name="user_id" class="appearance-none block w-full text-blue-900 border border-gray-600 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" type="text" placeholder=""> -->
                    </div>
                    <!-- +++ COL::group_id+++ -->
        ...
        <div class="flex-1 text-gray-700 text-left bg-gray-100 px-1 py-1 m-1">
            <div class="flex flox-grow">
                <div class="w-auto flex">
                    ...
                    <div class="w-60 underline font-bold text-blue-800 align-text-bottom">ユーザーID</div>
                    <div class="w-32 underline font-bold text-blue-800">グループID</div>
                </div>
                <div class=""></div>
            </div>
            @if (count($wip_user_groups) > 0)
                @foreach ($wip_user_groups as $wip_user_group)
                    <x-wip-user-group-collection id="{{ $wip_user_group->user_group_id }}">
                        <div class="flex">
                            <div class="w-20">{{ $wip_user_group->user_group_id }}</div>
                            <div class="w-60">{{ $wip_user_group->user_id }}</div> 
                            <div class="w-36">{{ $wip_user_group->group_id }}</div> 
                        </div>
                    </x-wip-user-group-collection>
                @endforeach
            @else
                <div>データがありません</div>
            @endif
